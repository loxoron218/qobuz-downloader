...
**Independent Test**: Type a song name in search bar → results appear within 3s grouped by tracks/albums/artists/playlists with cover art → select a track → choose quality → download begins → progress bar updates in real-time → file appears in the download directory with correct metadata tags and cover art.
...
- [X] T043 Handle remaining edge cases across modules:
  ...
  * Empty search results → display empty-state message with suggestion to try a different term (spec edge case #8)
  * API unavailable / rate-limited → delegate to FR-012 retry logic in worker; display toast if all retries exhausted
...
- T004 (src/types.rs) and T005 (src/preferences/settings.rs) (Phase 2): Quality enum and AppSettings — different files, no dependencies
